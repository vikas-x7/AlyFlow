...
export default function TrustedSection() {
  return (
    <section className="w-full bg-white py-12 sm:py-16 md:py-20 px-4 sm:px-6 font-gothic">
      <div className="max-w-6xl mx-auto">
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
          {stats.map((stat) => (
            <div
              key={stat.label}
              className="rounded-[10px] p-3 sm:p-4 flex flex-col justify-between min-h-36 sm:min-h-44 md:min-h-55"
              style={{ backgroundColor: "#F2F2F2" }}
            >
              <span className="text-[13px] sm:text-[15px] md:text-[17px] text-black font-medium tracking-wide">
                {stat.label}
              </span>
              <span className="text-4xl sm:text-5xl md:text-6xl font-light text-black/80 mt-4 sm:mt-6">
                {stat.value}
              </span>
            </div>
          ))}
        ...
        <div className="mt-20 sm:mt-32 md:mt-60">
          <h2 className="text-2xl sm:text-3xl md:text-4xl text-center font-medium text-black mb-8 sm:mb-12 font-instrument -tracking-[1px] sm:-tracking-[2px]">
            Why you should choose Alyflow
          </h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-10 lg:gap-x-16 gap-y-8 sm:gap-y-10 mt-10 sm:mt-16 md:mt-25">
            {features.map((f) => (
              <div key={f.title}>
                ...
                <p className="text-[14px] sm:text-[16px] font-semibold text-black/70 leading-relaxed">
                  {f.desc}
                </p>
              </div>
            ))}
